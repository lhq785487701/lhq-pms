...
$(function() {
	...
	// 移除数据
	methods_page.removeUser=function(evt,userCode){
		var index=-1;
		$.each(this.selectUsers,function(inx,tmp){
			if(tmp.user_code===userCode){
				index=inx;
				return false;
			}
		});
	...
	// 检查是否已经选了
	methods_page.isSelect=function(itm){
		var b=false;
		$.each(this.selectUsers,function(inx,tmp){
			if(tmp.user_code==itm.user_code){
				b=true;
				return false;
			}
		});
	...
	methods_page.saveUser=function(userCode){
		if(this.callback){
			...
			if(this.selectUsers && this.selectUsers.length>0){
				$.each(this.selectUsers,function(index,itm){
	...
	$.sendParams = function(obj) {
		...
		pageVue.selectUsers=obj.data.selectUsers;
